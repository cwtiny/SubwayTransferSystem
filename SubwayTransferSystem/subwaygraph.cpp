...
bool SubwayGraph::readFileData(QString fileName)
{
    ...
    while(!in.atEnd())
    {
        ...
        qDebug()<< "id:" << id << "; name:" << name << "; colour:" <<colour << ";";
        qDebug()<< "fromTo:" << fromTo << "; totalStations:" << totalStations << "; ok:" << ok;
        qDebug() << "flag:" << in.atEnd();
 ...
 * 参数：int s1 起点站索引（入参）
 *      int s2 终点站索引 (入参)
 *      QList<int>&stationsList  经过的站点集合（出参）
 *      QList<Edge>&edgesList    经过站点边的集合（出参）
*/
